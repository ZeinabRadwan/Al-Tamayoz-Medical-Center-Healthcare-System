<link rel="stylesheet" href="vv0all.css">

<style>
    @media print {
        .bcont {
            display: none !important;
        }

        aside {
            display: none !important;
        }

        header {
            display: none !important;
        }

        .inmain {
            width: 95% !important;
        }

        .container {
            width: 100% !important;
        }

        button {
            display: none;
        }

        /* تعيين الصفحة للطباعة في وضع landscape */
        @page {
            size: landscape;
        }
    }

    h1 {
        font-family: cairo;
        text-align: center;



        text-align: center;
    }

    .dko {

        margin: 20px;
        width: 15%;
        padding: 10px;
        border: none;
        border-radius: 5px;
        cursor: pointer;
        background: linear-gradient(0.25turn, #66AA3E, #92C641, #66AA3E);
        color: black;
        font-family: 'Droid Arabic Kufi', Arial, sans-serif;
        font-weight: bold;
        font-family: cairo;
        text-align: center;
    }
</style>
<link href="https://fonts.googleapis.com/css?family=Cairo" rel="stylesheet">


<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <style>
        * {
            font-family: Sans-serif;
        }

        .border {
            border: 2px solid black;
        }

        .row {
            display: flex;
            gap: 1rem;
            margin-bottom: 1rem;
        }

        .flex {
            display: flex;
        }

        .w-full {
            width: 100%;
        }

        .p-0 {
            padding: 0;
        }

        .font-bold {
            font-weight: bolder;
        }

        .font-w-none {
            font-weight: normal;
        }

        .size-fit {
            min-width: 820px;
            max-width: 1060px;
        }

        .p-fit {
            padding: 2rem 3rem;
        }

        table {
            border-collapse: collapse;
            width: 100%;
        }

        th,
        td {
            border: 1px solid black;
            padding: 8px;
            text-align: left;
        }

        .t-center {
            text-align: center;
        }

        .bg-grey {
            background-color: rgb(238, 238, 238);
        }

        .t-grey {
            color: #a7a4a4;
        }

        .RangeOfMotionTable th,
        td {
            padding: 0 4px;
        }

        .b-none {
            border: none;
        }

        .input readonly readonly-none {
            border: none;
            outline: none;
        }

        .h-w-full {
            height: 100%;
            width: 100%;
            background-color: #f0f0f0;
        }

        .bg-yellow {
            background-color: rgb(249, 249, 147);
        }

        th,
        td {
            padding: 0 0px;
            height: 30px;
            background-color: #f0f0f0 !important;
            height: 30px;
        }

        .b-none {
            border: none;
            color: darkgreen;
        }
    </style>
</head>

<form>

    <body style="display: flex; flex-direction: column; gap: 2rem; align-items: center;">

        <!DOCTYPE html>

        <title>Document</title>
        <style>
            * {
                font-family: Sans-serif;
            }

            .border {
                border: 2px solid black;
            }

            .row {
                display: flex;
                gap: 1rem;
                margin-bottom: 1rem;
            }

            .flex {
                display: flex;
            }

            .w-full {
                width: 100%;
            }

            .p-0 {
                padding: 0;
            }

            .font-bold {
                font-weight: bolder;
            }

            .font-w-none {
                font-weight: normal;
            }

            .size-fit {
                min-width: 820px;
                max-width: 1060px;
            }

            .p-fit {
                padding: 2rem 1rem;
            }

            table {
                border-collapse: collapse;
                width: 100%;
            }

            th,
            td {
                border: 1px solid black;
                padding: 8px;
                text-align: left;
            }

            .t-center {
                text-align: center;
            }

            .bg-grey {
                background-color: rgb(238, 238, 238);
            }

            .RangeOfMotionTable th,
            td {
                padding: 0 4px;
            }
        </style>

        <body style="display: flex; flex-direction: column; gap: 2rem; align-items: center;">
            <div class="border size-fit p-fit">

                <h3>
                    PEDIATRECS PHYSIO THERAPY ASSESSMENT
                </h3>
                <div style="display: flex; flex-wrap: wrap; gap: 1.5rem; margin-bottom: 1rem;">
                    <div style="flex: 2;">
                        <label>Patient Name:</label>
                        <input readonly readonly style="width: 100%; margin-bottom: 0.5rem;" type="text" value="{{ $report->patient_name }}" name="patient_name">
                        <label>Diagnosis:</label>
                        <input readonly readonly style="width: 100%;" type="text" value="{{ $report->diagnosis }}" name="diagnosis">
                    </div>
                    <div style="flex: 1;">
                        <label>Nationality:</label>
                        <input readonly readonly style="width: 100%; margin-bottom: 0.5rem;" type="text" value="{{ $report->nationality }}" name="nationality">
                        <label>Patient Age:</label>
                        <input readonly readonly style="width: 100%;" type="text" value="{{ $report->patient_age }}" name="patient_age">
                    </div>
                    <div style="flex: 2;">
                        <label>Occupation:</label>
                        <input readonly readonly style="width: 100%; margin-bottom: 0.5rem;" type="text" value="{{ $report->occupation }}" name="occupation">
                        <label>ID/MRN:</label>
                        <input readonly readonly style="width: 100%;" type="text" value="{{ $report->id_mrn }}" name="id_mrn">
                    </div>

                    <div style="flex:1 ;">
                        <label>Gender:</label>
                        <input readonly readonly type="checkbox" onclick="return false;" onclick="return false;" id="female" value="1" name="gender_female" {{ $report->gender_female ? 'checked' : '' }}>
                        <label for="female">F</label>
                        <input readonly readonly type="checkbox" onclick="return false;" onclick="return false;" id="male" value="1" name="gender_male" {{ $report->gender_male ? 'checked' : '' }}>
                        <label for="male">M</label>
                    </div>

                </div>
                <table>

                    <tr>
                        <th>2.</th>
                        <td>History of trauma/ illness</td>
                        <td colspan="2" align=center style='border-left: 1px solid black;'>
                            <textarea name="history_of_trauma_illness" rows="3" style="width: 100%;">{{ $report->history_of_trauma_illness }}</textarea>
                        </td>
                        <td>
                            <label for='history_of_trauma_illness_date'>Date</label>
                            <input readonly readonly type='text' id='history_of_trauma_illness_date' value="{{ $report->history_of_trauma_illness_date }}" name='history_of_trauma_illness_date'>
                        </td>
                    </tr>
                    <tr>
                        <th>3.</th>
                        <td>Medical History/Chief Complain:</td>
                        <td colspan="4" align=center style='border-left: 1px solid black;'>
                            <textarea name="medical_history_chief_complain" rows="3" style="width: 100%;">{{ $report->medical_history_chief_complain }}</textarea>
                        </td>
                    </tr>
                    <tr>
                        <th>4.</th>
                        <td>X-ray/ Other ex:</td>
                        <td colspan="4" align=center style='border-left: 1px solid black;'>
                            <textarea name="xray_other_ex" rows="3" style="width: 100%;">{{ $report->xray_other_ex }}</textarea>
                        </td>
                    </tr>
                    <tr>
                        <th>5.</th>
                        <td>Pain level VAS</td>
                        <td colspan="4" align=center style='border-left: 1px solid black;'>
                            <textarea name="pain_level_vas" rows="3" style="width: 100%;">{{ $report->pain_level_vas }}</textarea>
                        </td>
                    </tr>
                </table>
            </div>
            <div class="border size-fit p-fit">
                <div style="margin-right: 2rem; margin-top: 1rem;">
                    <h3>Examination and Assessment:</h3>
                    <h5>
                        Physical Examination: Mark on the body-chart deformities or joint anomalies, back deformities or anomalies, edema, shoulder subluxatio
                    </h5>
                </div>
                <div style="display: flex; justify-content: center;">
                    <img src="data:image/png;base64,"
                        alt="Not found">
                </div>
                </table>
            </div>
            <div>
                <h5 style="margin: 1rem 0;">Skin, Soft tissues problem</h5>
                <table>
                    <tr>
                        <th class="bg-grey">DISORDERS</th>
                        <th class="bg-grey">Right</th>
                        <th class="bg-grey">Left</th>
                    </tr>
                    <tr>
                        <th class="bg-grey">Swelling</th>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->swelling_right }}" name="swelling_right"></td>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->swelling_left }}" name="swelling_left"></td>
                    </tr>
                    <tr>
                        <th class="bg-grey">Callus</th>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->callus_right }}" name="callus_right"></td>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->callus_left }}" name="callus_left"></td>
                    </tr>
                    <tr>
                        <th class="bg-grey">Scar</th>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->scar_right }}" name="scar_right"></td>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->scar_left }}" name="scar_left"></td>
                    </tr>
                    <tr>
                        <th class="bg-grey">Wound</th>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->wound_right }}" name="wound_right"></td>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->wound_left }}" name="wound_left"></td>
                    </tr>
                    <tr>
                        <th class="bg-grey">Temperature</th>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->temperature_right }}" name="temperature_right"></td>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->temperature_left }}" name="temperature_left"></td>
                    </tr>
                    <tr>
                        <th class="bg-grey">Infection</th>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->infection_right }}" name="infection_right"></td>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->infection_left }}" name="infection_left"></td>
                    </tr>
                    <tr>
                        <th class="bg-grey">Pain</th>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->pain_right }}" name="pain_right"></td>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->pain_left }}" name="pain_left"></td>
                    </tr>
                    <tr>
                        <th class="bg-grey">Abnormal Sensation</th>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->abnormal_sensation_right }}" name="abnormal_sensation_right"></td>
                        <td><input readonly readonly type="text" style="width: 95%;" value="{{ $report->abnormal_sensation_left }}" name="abnormal_sensation_left"></td>
                    </tr>
                    <tr>
                        <td>Extension 0</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->extension_0_1 }}" name="extension_0_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->extension_0_2 }}" name="extension_0_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->extension_0_3 }}" name="extension_0_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->extension_0_4 }}" name="extension_0_4"></td>
                    </tr>
                    <tr>
                        <td>ANKLE-FOOT</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->ankle_foot_1 }}" name="ankle_foot_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->ankle_foot_2 }}" name="ankle_foot_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->ankle_foot_3 }}" name="ankle_foot_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->ankle_foot_4 }}" name="ankle_foot_4"></td>
                    </tr>
                    <tr>
                        <td>Dorsi Flexion 30</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->dorsi_flexion_1 }}" name="dorsi_flexion_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->dorsi_flexion_2 }}" name="dorsi_flexion_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->dorsi_flexion_3 }}" name="dorsi_flexion_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->dorsi_flexion_4 }}" name="dorsi_flexion_4"></td>
                    </tr>
                    <tr>
                        <td>Plantar Flexion 45</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->plantar_flexion_1 }}" name="plantar_flexion_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->plantar_flexion_2 }}" name="plantar_flexion_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->plantar_flexion_3 }}" name="plantar_flexion_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->plantar_flexion_4 }}" name="plantar_flexion_4"></td>
                    </tr>
                    <tr>
                        <td>Inversion 35</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->inversion_1 }}" name="inversion_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->inversion_2 }}" name="inversion_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->inversion_3 }}" name="inversion_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->inversion_4 }}" name="inversion_4"></td>
                    </tr>
                    <tr>
                        <td>Eversion 15</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->eversion_1 }}" name="eversion_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->eversion_2 }}" name="eversion_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->eversion_3 }}" name="eversion_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->eversion_4 }}" name="eversion_4"></td>
                    </tr>
                    <tr>
                        <th colspan="5">NECK</th>
                    </tr>
                    <tr>
                        <td>Flexion cm</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_flexion_1 }}" name="neck_flexion_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_flexion_2 }}" name="neck_flexion_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_flexion_3 }}" name="neck_flexion_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_flexion_4 }}" name="neck_flexion_4"></td>
                    </tr>
                    <tr>
                        <td>Extension cm</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_extension_1 }}" name="neck_extension_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_extension_2 }}" name="neck_extension_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_extension_3 }}" name="neck_extension_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_extension_4 }}" name="neck_extension_4"></td>
                    </tr>
                    <tr>
                        <td>Latero-Flexion R cm</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_latero_flexion_r_1 }}" name="neck_latero_flexion_r_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_latero_flexion_r_2 }}" name="neck_latero_flexion_r_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_latero_flexion_r_3 }}" name="neck_latero_flexion_r_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_latero_flexion_r_4 }}" name="neck_latero_flexion_r_4"></td>
                    </tr>
                    <tr>
                        <td>Latero-Flexion L cm</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_latero_flexion_l_1 }}" name="neck_latero_flexion_l_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_latero_flexion_l_2 }}" name="neck_latero_flexion_l_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_latero_flexion_l_3 }}" name="neck_latero_flexion_l_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->neck_latero_flexion_l_4 }}" name="neck_latero_flexion_l_4"></td>
                    </tr>
                    <tr>
                        <th colspan="5">TRUNK</th>
                    </tr>
                    <tr>
                        <td>Global Flexion cm</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->global_flexion_1 }}" name="global_flexion_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->global_flexion_2 }}" name="global_flexion_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->global_flexion_3 }}" name="global_flexion_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->global_flexion_4 }}" name="global_flexion_4"></td>
                    </tr>
                    <tr>
                        <td style="font-size: 10px;">Thoracic Flexion (Ott Test) cm</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->thoracic_flexion_1 }}" name="thoracic_flexion_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->thoracic_flexion_2 }}" name="thoracic_flexion_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->thoracic_flexion_3 }}" name="thoracic_flexion_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->thoracic_flexion_4 }}" name="thoracic_flexion_4"></td>
                    </tr>
                    <tr>
                        <td style="font-size: 10px;">Lumbar Flexion (Schober test)</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->lumbar_flexion_1 }}" name="lumbar_flexion_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->lumbar_flexion_2 }}" name="lumbar_flexion_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->lumbar_flexion_3 }}" name="lumbar_flexion_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->lumbar_flexion_4 }}" name="lumbar_flexion_4"></td>
                    </tr>
                    <tr>
                        <td>Global Extension cm</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->global_extension_1 }}" name="global_extension_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->global_extension_2 }}" name="global_extension_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->global_extension_3 }}" name="global_extension_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->global_extension_4 }}" name="global_extension_4"></td>
                    </tr>
                    <tr>
                        <td>Latero-Flexion R cm</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->latero_flexion_r_1 }}" name="latero_flexion_r_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->latero_flexion_r_2 }}" name="latero_flexion_r_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->latero_flexion_r_3 }}" name="latero_flexion_r_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->latero_flexion_r_4 }}" name="latero_flexion_r_4"></td>
                    </tr>
                    <tr>
                        <td>Latero-Flexion L cm</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->latero_flexion_l_1 }}" name="latero_flexion_l_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->latero_flexion_l_2 }}" name="latero_flexion_l_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->latero_flexion_l_3 }}" name="latero_flexion_l_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->latero_flexion_l_4 }}"></td>
                    </tr>
                    <tr>
                        <td style="font-size: 10px;">Rotation R (write OK or imp.)</td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->rotation_r_1 }}" name="rotation_r_1"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->rotation_r_2 }}" name="rotation_r_2"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->rotation_r_3 }}" name="rotation_r_3"></td>
                        <td><input readonly readonly style="width:50px;" type="text" value="{{ $report->rotation_r_4 }}" name="rotation_r_4"></td>
                    </tr>
                    <tr>
                        <td style="font-size: 10px;">Rotation L (write OK or imp.)</td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->rotation_l_1 }}" name="rotation_l_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->rotation_l_2 }}" name="rotation_l_2"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->rotation_l_3 }}" name="rotation_l_3"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->rotation_l_4 }}" name="rotation_l_4"></td>
                    </tr>
                    <tr>
                        <td style="font-size: 10px;">Supination 80</td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->forearm_supination_l_1 }}" name="forearm_supination_l_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->forearm_supination_r_1 }}" name="forearm_supination_r_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->forearm_supination_l_2 }}" name="forearm_supination_l_2"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->forearm_supination_r_2 }}" name="forearm_supination_r_2"></td>
                    </tr>
                    <tr>
                        <th colspan="5">WRIST</th>
                    </tr>
                    <tr>
                        <td style="font-size: 10px;">Flexion 80</td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_flexion_l_1 }}" name="wrist_flexion_l_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_flexion_r_1 }}" name="wrist_flexion_r_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_flexion_l_2 }}" name="wrist_flexion_l_2"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_flexion_r_2 }}" name="wrist_flexion_r_2"></td>
                    </tr>
                    <tr>
                        <td style="font-size: 10px;">Extension 80</td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_extension_l_1 }}" name="wrist_extension_l_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_extension_r_1 }}" name="wrist_extension_r_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_extension_l_2 }}" name="wrist_extension_l_2"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_extension_r_2 }}" name="wrist_extension_r_2"></td>
                    </tr>
                    <tr>
                        <td style="font-size: 10px;">Abduction 20</td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_abduction_l_1 }}" name="wrist_abduction_l_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_abduction_r_1 }}" name="wrist_abduction_r_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_abduction_l_2 }}" name="wrist_abduction_l_2"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_abduction_r_2 }}" name="wrist_abduction_r_2"></td>
                    </tr>
                    <tr>
                        <td style="font-size: 10px;">Adduction 35</td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_adduction_l_1 }}" name="wrist_adduction_l_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_adduction_r_1 }}" name="wrist_adduction_r_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_adduction_l_2 }}" name="wrist_adduction_l_2"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->wrist_adduction_r_2 }}" name="wrist_adduction_r_2"></td>
                    </tr>
                    <tr>
                        <th colspan="5">FINGERS</th>
                    </tr>
                    <tr>
                        <td style="font-size: 10px;">Thumb opposition</td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_thumb_opposition_l_1 }}" name="finger_thumb_opposition_l_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_thumb_opposition_r_1 }}" name="finger_thumb_opposition_r_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_thumb_opposition_l_2 }}" name="finger_thumb_opposition_l_2"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_thumb_opposition_r_2 }}" name="finger_thumb_opposition_r_2"></td>
                    </tr>
                    <tr>
                        <td style="font-size: 10px;">MP Flexion 90</td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_mp_flexion_l_1 }}" name="finger_mp_flexion_l_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_mp_flexion_r_1 }}" name="finger_mp_flexion_r_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_mp_flexion_l_2 }}" name="finger_mp_flexion_l_2"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_mp_flexion_r_2 }}" name="finger_mp_flexion_r_2"></td>
                    </tr>
                    <tr>
                        <td style="font-size: 10px;">MP Extension 40</td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_mp_extension_l_1 }}" name="finger_mp_extension_l_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_mp_extension_r_1 }}" name="finger_mp_extension_r_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_mp_extension_l_2 }}" name="finger_mp_extension_l_2"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_mp_extension_r_2 }}" name="finger_mp_extension_r_2"></td>
                    </tr>
                    <tr>
                        <td style="font-size: 10px;">IP Flexion 120</td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_ip_flexion_l_1 }}" name="finger_ip_flexion_l_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_ip_flexion_r_1 }}" name="finger_ip_flexion_r_1"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_ip_flexion_l_2 }}" name="finger_ip_flexion_l_2"></td>
                        <td><input readonly readonly style="width: 50px;" type="text" value="{{ $report->finger_ip_flexion_r_2 }}" name="finger_ip_flexion_r_2"></td>
                    </tr>
                    </tbody>
                </table>
                <br><br>
                <label for="Remarks5">Remarks:</label><br>
                <textarea name="remarks" id="Remarks5" cols="40" rows="6">{{ $report->remarks }}</textarea>
            </div>
            </div>
            </div>
            <div class="border size-fit p-fit">
                <div style="display: flex; gap: .5rem;">
                    <div style="width: 295px;">
                        <h3 style="font-size: 15px;">Muscle Test:</h3>
                        <p style="font-size: 13px;">
                            <input readonly readonly type="checkbox" onclick="return false;" onclick="return false;" value='1' name="muscle_test" id="RangeOfMotion"
                                {{ $report-> muscle_test ? 'checked' : '' }}>Muscle test should be
                            recorded during first
                            assessment and before
                            discharging the patient
                        </p>
                    </div>
                    <div>
                        <table style="width: 100%; font-size: 12px;" class="RangeOfMotionTable" border="1">
                            <tbody>
                                <tr style="height: 60px; width: 100%;">
                                    <th colspan="3">LOWER LIMB</th>
                                    <th rowspan="1" class="t-center" colspan="4">RT<input readonly readonly
                                            style="border: 1px solid rgb(10, 226, 10); width: 100%;" type="text" value="{{ $report->lower_limb_rt }}" name="lower_limb_rt">
                                    </th>
                                    <th rowspan="1" class="t-center" colspan="4">LT <input readonly readonly
                                            style="border: 1px solid rgb(10, 226, 10); width: 55%;" type="text" value="{{ $report->lower_limb_lt }}" name="lower_limb_lt">
                                    </th>
                                </tr>
                            </tbody>
                        </table>
                        <br />
                        <span class="mt-3"> Neck:
                            <input readonly readonly type="text" value="{{ $report->neck_span }}" name="neck_span">
                        </span>
                    </div>
                    <div>
                        <table style="" class="RangeOfMotionTable" border="1">
                            <tbody>
                                <table style="width: 100%; font-size: 12px;" class="RangeOfMotionTable" border="1">
                                    <tbody class="mb-4">
                                        <tr style="height: 60px; width: 100%;">
                                            <th colspan="3">Upper LIMB</th>
                                            <th rowspan="1" class="t-center" colspan="4">RT<input readonly readonly
                                                    style="border: 1px solid rgb(10, 226, 10); width: 100%;" type="text" value="{{ $report->upper_limb_rt }}" name="upper_limb_rt">
                                            </th>
                                            <th rowspan="1" class="t-center" colspan="4">LT <input readonly readonly
                                                    style="border: 1px solid rgb(10, 226, 10); width: 100%;" type="text" value="{{ $report->upper_limb_lt }}" name="upper_limb_lt">
                                            </th>
                                        </tr>
                                    </tbody>
                                </table>
                                <br />
                                <span> Trunk:
                                    <input readonly readonly class="mt-3" type="text" value="{{ $report->trunk_span }}" name="trunk_span">
                                </span>
                    </div>
                </div>
            </div>
            <div class="border size-fit p-fit">
                <div style="display: flex; gap: .5rem;">
                    <div style="width: 295px; display: flex;">
                        <h3 style="font-size: 15px;">Muscle Tone:</h3>
                        <p style="font-size: 13px;">
                            <input readonly readonly type="checkbox" onclick="return false;" onclick="return false;" value='1' name="hypertonia" id="hypertonia" {{ $report->hypertonia ? 'checked' : '' }}>
                            Hypertonia

                            <input readonly readonly type="checkbox" onclick="return false;" onclick="return false;" value='1' name="hypotonia" id="hypotonia" {{ $report->hypotonia ? 'checked' : '' }}>
                            Hypotonia
                        </p>
                    </div>
                </div>
            </div>
            <div class="border size-fit p-fit">
                <h3>Functional Evaluation:</h3>
                <div style="display: flex; margin-left: 4rem; gap: 3rem;">
                    <div>
                        <h4>Balance</h4>
                        <table class="RangeOfMotionTable" border="1">
                            <tbody>
                                <tr>
                                    <th class="t-center" rowspan="4">Sitting Static</th>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="static_sitting_normal" {{ $report->static_sitting_normal ? 'checked' : '' }}> Normal</th>
                                </tr>
                                <tr>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="static_sitting_good" {{ $report->static_sitting_good ? 'checked' : '' }}> Good</th>
                                </tr>
                                <tr>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="static_sitting_poor" {{ $report->static_sitting_poor ? 'checked' : '' }}> Poor</th>
                                </tr>
                                <tr>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="static_sitting_not_possible" {{ $report->static_sitting_not_possible ? 'checked' : '' }}> Not Possible</th>
                                </tr>
                                <tr>
                                    <th class="t-center" rowspan="4">Standing Static</th>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="static_standing_normal" {{ $report->static_standing_normal ? 'checked' : '' }}> Normal</th>
                                </tr>
                                <tr>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="static_standing_good" {{ $report->static_standing_good ? 'checked' : '' }}> Good</th>
                                </tr>
                                <tr>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="static_standing_poor" {{ $report->static_standing_poor ? 'checked' : '' }}> Poor</th>
                                </tr>
                                <tr>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="sstatic_tanding_not_possible" {{ $report->sstatic_tanding_not_possible ? 'checked' : '' }}> Not Possible</th>
                                </tr>

                                <tr>
                                    <th class="t-center" rowspan="4">Dynamic Static</th>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="dynamic_sitting_normal" {{ $report->dynamic_sitting_normal ? 'checked' : '' }}> Normal</th>
                                </tr>
                                <tr>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="dynamic_sitting_good" {{ $report->dynamic_sitting_good ? 'checked' : '' }}> Good</th>
                                </tr>
                                <tr>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="dynamic_sitting_poor" {{ $report->dynamic_sitting_poor ? 'checked' : '' }}> Poor</th>
                                </tr>
                                <tr>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="dynamic_sitting_not_possible" {{ $report->dynamic_sitting_not_possible ? 'checked' : '' }}> Not Possible</th>
                                </tr>
                                <tr>
                                    <th class="t-center" rowspan="4">Dynamic Static</th>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="dynamic_standing_normal" {{ $report->dynamic_standing_normal ? 'checked' : '' }}> Normal</th>
                                </tr>
                                <tr>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="dynamic_standing_good" {{ $report->dynamic_standing_good ? 'checked' : '' }}> Good</th>
                                </tr>
                                <tr>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="dynamic_standing_poor" {{ $report->dynamic_standing_poor ? 'checked' : '' }}> Poor</th>
                                </tr>
                                <tr>
                                    <th class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="dynamic_standing_not_possible" {{ $report->dynamic_standing_not_possible ? 'checked' : '' }}> Not Possible</th>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <div>
                        <h4>Coordination:</h4>
                        <table class="RangeOfMotionTable" border="1">
                            <tbody>
                                <tr>
                                    <th rowspan="2" class="t-center">UPPER LIMBS</th>
                                    <th colspan="2" class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="upper_limbs_good" {{ $report->upper_limbs_good ? 'checked' : '' }}> Good</th>
                                    <th colspan="2" class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="upper_limbs_poor" {{ $report->upper_limbs_poor ? 'checked' : '' }}> Poor</th>
                                    <th colspan="2" class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="upper_limbs_not_possible" {{ $report->upper_limbs_not_possible ? 'checked' : '' }}> Not Possible</th>
                                </tr>
                                <tr>
                                    <th>L</th>
                                    <th>R</th>
                                    <th>L</th>
                                    <th>R</th>
                                    <th>L</th>
                                    <th>R</th>
                                </tr>
                                <tr>
                                    <th rowspan="2" class="t-center">LOWER LIMBS</th>
                                    <th colspan="2" class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="lower_limbs_good" {{ $report->lower_limbs_good ? 'checked' : '' }}> Good</th>
                                    <th colspan="2" class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="lower_limbs_poor" {{ $report->lower_limbs_poor ? 'checked' : '' }}> Poor</th>
                                    <th colspan="2" class="t-center"><input readonly readonly class="keda" type="checkbox" onclick="return false;" onclick="return false;" value="1" name="lower_limbs_not_possible" {{ $report->lower_limbs_not_possible ? 'checked' : '' }}> Not Possible</th>
                                </tr>
                                <tr>
                                    <th>L</th>
                                    <th>R</th>
                                    <th>L</th>
                                    <th>R</th>
                                    <th>L</th>
                                    <th>R</th>
                                </tr>
                                <tr>
                                    <th>
                                        Comments
                                    </th>
                                    <td colspan="6">
                                        <textarea name="coordination_comments" cols="30" rows="3">{{ $report->coordination_comments }}</textarea>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>

            <div class="border size-fit p-fit">
                <h3>Activity Limitations Participation Restrictions:</h3>
                <table style="font-size: 10px;">
                    <tbody>
                        <tr>
                            <td style="border: none;"></td>
                            <th colspan="2" class="t-center">ACTIVITIES/ PARTICIPATIONS</th>
                            <th class="t-center">Independent</th>
                            <th class="t-center">Assisted</th>
                            <th class="t-center">Impossible</th>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;">MOBILITY</th>
                            <td><input readonly type="text" value="{{ $report->i0 }}" name="i0"></td>
                            <th>Crawling</th>
                            <!-- hidden input readonly -->
                            <input readonly type="hidden" value="{{ $report->i1 }}" name="i1">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i1" {{ $report->i1 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i2 }}" name="i2">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i2" {{ $report->i2 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i3 }}" name="i3">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i3" {{ $report->i3 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i4 }}" name="i4"></td>
                            <th>Crouching gait</th>

                            <input readonly type="hidden" value="{{ $report->i5 }}" name="i5">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i5" {{ $report->i5 ? 'checked' : '' }}></td>

                            <input readonly type="hidden" value="{{ $report->i6 }}" name="i6">

                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i6" {{ $report->i6 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i7 }}" name="i7">

                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i7" {{ $report->i7 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i8 }}" name="i8"></td>
                            <th>Walking</th>
                            <input readonly type="hidden" value="{{ $report->i9 }}" name="i9">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i9" {{ $report->i9 ? 'checked' : '' }}></td>

                            <input readonly type="hidden" value="{{ $report->i10 }}" name="i10">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i10" {{ $report->i10 ? 'checked' : '' }}></td>

                            <input readonly type="hidden" value="{{ $report->i11 }}" name="i11">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i11" {{ $report->i11 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i12 }}" name="i12"></td>
                            <th>Squatting</th>

                            <input readonly type="hidden" value="{{ $report->i13 }}" name="i13">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i13" {{ $report->i13 ? 'checked' : '' }}></td>

                            <input readonly type="hidden" value="{{ $report->i14 }}" name="i14">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i14" {{ $report->i14 ? 'checked' : '' }}></td>

                            <input readonly type="hidden" value="{{ $report->i15 }}" name="i15">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i15" {{ $report->i15 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i17 }}" name="i17"></td>
                            <th>Stairs</th>
                            <input readonly type="hidden" value="{{ $report->i18 }}" name="i18">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i18" {{ $report->i18 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i19 }}" name="i19">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i19" {{ $report->i19 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i20 }}" name="i20">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i20" {{ $report->i20 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i21 }}" name="i21"></td>
                            <th>Running</th>
                            <input readonly type="hidden" value="{{ $report->i22 }}" name="i22">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i22" {{ $report->i22 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i23 }}" name="i23">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i23" {{ $report->i23 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i24 }}" name="i24">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i24" {{ $report->i24 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;">TRANSFERS</th>
                            <td><input readonly type="text" value="{{ $report->i25 }}" name="i25"></td>
                            <th>Lie to Sit (opposite)</th>
                            <input readonly type="hidden" value="{{ $report->i26 }}" name="i26">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i26" {{ $report->i26 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i27 }}" name="i27">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i27" {{ $report->i27 ? 'checked' : '' }}></td>
                            <td><input readonly type="text" value="{{ $report->i28 }}" name="i28"></td>
                            <th>Sit to Stand (opposite)</th>
                            <input readonly type="hidden" value="{{ $report->i29 }}" name="i29">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i29" {{ $report->i29 ? 'checked' : '' }}></td>

                            <input readonly type="hidden" value="{{ $report->i30 }}" name="i30">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i30" {{ $report->i30 ? 'checked' : '' }}></td>

                            <input readonly type="hidden" value="{{ $report->i31 }}" name="i31">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i31" {{ $report->i31 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i32 }}" name="i32"></td>
                            <th>Stand to Floor (opposite)</th>
                            <input readonly type="hidden" value="{{ $report->i33 }}" name="i33">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i33" {{ $report->i33 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i34 }}" name="i34">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i34" {{ $report->i34 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i35 }}" name="i35">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i35" {{ $report->i35 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i36 }}" name="i36"></td>
                            <th>Sit to sit</th>
                            <input readonly type="hidden" value="{{ $report->i37 }}" name="i37">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i37" {{ $report->i37 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i38 }}" name="i38">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i38" {{ $report->i38 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i39 }}" name="i39">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i39" {{ $report->i39 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;">BALANCE</th>
                            <td><input readonly type="text" value='{{ $report->i40 }} ' name="i40"></td>
                            <th>Sitting</th>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i41 }}" name="i41"><input readonly type="checkbox" onclick="return false;" value="1" name="i41" {{ $report->i41 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i42 }}" name="i42"><input readonly type="checkbox" onclick="return false;" value="1" name="i42" {{ $report->i42 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i43 }}" name="i43"><input readonly type="checkbox" onclick="return false;" value="1" name="i43" {{ $report->i43 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i44 }}" name="i44"></td>
                            <th>Standing</th>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i45 }}" name="i45"><input readonly type="checkbox" onclick="return false;" value="1" name="i45" {{ $report->i45 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i46 }}" name="i46"><input readonly type="checkbox" onclick="return false;" value="1" name="i46" {{ $report->i46 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i47 }}" name="i47"><input readonly type="checkbox" onclick="return false;" value="1" name="i47" {{ $report->i47 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i48 }}" name="i48"></td>
                            <th>On one leg</th>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i49 }}" name="i49"><input readonly type="checkbox" onclick="return false;" value="1" name="i49" {{ $report->i49 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i50 }}" name="i50"><input readonly type="checkbox" onclick="return false;" value="1" name="i50" {{ $report->i50 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i51 }}" name="i51"><input readonly type="checkbox" onclick="return false;" value="1" name="i51" {{ $report->i51 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;">UPPER LIMB</th>
                            <td><input readonly type="text" value="{{ $report->i52 }}" name="i52"></td>
                            <th rowspan="2">
                                <div style="justify-content: space-between; position: relative;" class="flex">
                                    <div>Grasp </div>
                                    <div
                                        style="border: 1px solid black; position: absolute; right: -9px; width: 40px; height: 53px; top: -22px; display: flex; flex-direction: column; justify-content: space-between;">
                                        <div style="padding: 7px;">L</div>
                                        <div style="border-top: 1px solid black; padding: 7px;">R</div>
                                    </div>
                                </div>
                            </th>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i53 }}" name="i53"><input readonly type="checkbox" onclick="return false;" value="1" name="i53" {{ $report->i53 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i54 }}" name="i54"><input readonly type="checkbox" onclick="return false;" value="1" name="i54" {{ $report->i54 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i55 }}" name="i55"><input readonly type="checkbox" onclick="return false;" value="1" name="i55" {{ $report->i55 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;">FUNCTIO</th>
                            <td><input readonly type="text" value="{{ $report->i56 }}" name="i56"></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i57 }}" name="i57"><input readonly type="checkbox" onclick="return false;" value="1" name="i57" {{ $report->i57 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i58 }}" name="i58"><input readonly type="checkbox" onclick="return false;" value="1" name="i58" {{ $report->i58 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i59 }}" name="i59"><input readonly type="checkbox" onclick="return false;" value="1" name="i59" {{ $report->i59 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i60 }}" name="i60"></td>
                            <th rowspan="2">
                                <div style="justify-content: space-between; position: relative;" class="flex">
                                    <div>Release </div>
                                    <div
                                        style="border: 1px solid black; position: absolute; right: -9px; width: 40px; height: 43px; top: -17px; display: flex; flex-direction: column; justify-content: space-between;">
                                        <div style="padding: 7px;">L</div>
                                        <div style="border-top: 1px solid black; padding: 7px;">R</div>
                                    </div>
                                </div>
                            </th>
                            <input readonly type="hidden" value="{{ $report->i61 }}" name="i61">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i61" {{ $report->i61 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i62 }}" name="i62">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i62" {{ $report->i62 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i63 }}" name="i63">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i63" {{ $report->i63 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i64 }}" name="i64"></td>
                            <input readonly type="hidden" value="{{ $report->i65 }}" name="i65">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i65" {{ $report->i65 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i66 }}" name="i66">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i66" {{ $report->i66 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i67 }}" name="i67">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i67" {{ $report->i67 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i68 }}" name="i68"></td>
                            <th rowspan="2">
                                <div style="justify-content: space-between; position: relative;" class="flex">
                                    <div>Fine Manipulation</div>
                                    <div
                                        style="border: 1px solid black; position: absolute; right: -9px; width: 40px; height: 43px; top: -17px; display: flex; flex-direction: column; justify-content: space-between;">
                                        <div style="padding: 7px;">L</div>
                                        <div style="border-top: 1px solid black; padding: 7px;">R</div>
                                    </div>
                                </div>
                            </th>
                            <input readonly type="hidden" value="{{ $report->i69 }}" name="i69">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i69" {{ $report->i69 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i70 }}" name="i70">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i70" {{ $report->i70 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i71 }}" name="i71">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i71" {{ $report->i71 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i72 }}" name="i72"></td>
                            <input readonly type="hidden" value="{{ $report->i73 }}" name="i73">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i73" {{ $report->i73 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i74 }}" name="i74">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i74" {{ $report->i74 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i75 }}" name="i75">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i75" {{ $report->i75 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i76 }}" name="i76"></td>
                            <th rowspan="2">
                                <div style="justify-content: space-between; position: relative;" class="flex">
                                    <div>Holding </div>
                                    <div
                                        style="border: 1px solid black; position: absolute; right: -9px; width: 40px; height: 43px; top: -17px; display: flex; flex-direction: column; justify-content: space-between;">
                                        <div style="padding: 7px;">L</div>
                                        <div style="border-top: 1px solid black; padding: 7px;">R</div>
                                    </div>
                                </div>
                            </th>
                            <input readonly type="hidden" value="{{ $report->i77 }}" name="i77">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i77" {{ $report->i77 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i78 }}" name="i78">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i78" {{ $report->i78 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i79 }}" name="i79">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i79" {{ $report->i79 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i77" {{ $report->i77 ? 'checked' : '' }}></td>
                        <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i78" {{ $report->i78 ? 'checked' : '' }}></td>
                        <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i79" {{ $report->i79 ? 'checked' : '' }}></td>
                        <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i80 }}" name="i80"></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i81 }}" name="i81"><input readonly type="checkbox" onclick="return false;" value="1" name="i81" {{ $report->i81 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i82 }}" name="i82"><input readonly type="checkbox" onclick="return false;" value="1" name="i82" {{ $report->i82 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i83 }}" name="i83"><input readonly type="checkbox" onclick="return false;" value="1" name="i83" {{ $report->i83 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i84 }}" name="i84"></td>
                            <th rowspan="2">
                                <div style="justify-content: space-between; position: relative;" class="flex">
                                    <div><input readonly type="text" value="{{ $report->i85 }}" name="i85" style="width: 50%"></div>
                                    <div
                                        style="border: 1px solid black; position: absolute; right: -9px; width: 40px; height: 43px; top: -12px; display: flex; flex-direction: column; justify-content: space-between;">
                                        <div style="padding: 7px;">L</div>
                                        <div style="border-top: 1px solid black; padding: 7px;">R</div>
                                    </div>
                                </div>
                            </th>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i86 }}" name="i86"><input readonly type="checkbox" onclick="return false;" value="1" name="i86" {{ $report->i86 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i87 }}" name="i87"><input readonly type="checkbox" onclick="return false;" value="1" name="i87" {{ $report->i87 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i88 }}" name="i88"><input readonly type="checkbox" onclick="return false;" value="1" name="i88" {{ $report->i88 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i89 }}" name="i89"></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i90 }}" name="i90"><input readonly type="checkbox" onclick="return false;" value="1" name="i90" {{ $report->i90 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i91 }}" name="i91"><input readonly type="checkbox" onclick="return false;" value="1" name="i91" {{ $report->i91 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i92 }}" name="i92"><input readonly type="checkbox" onclick="return false;" value="1" name="i92" {{ $report->i92 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;">DAILY LIFE</th>
                            <td><input readonly type="text" value="{{ $report->i93 }}" name="i93"></td>
                            <th>Dressing - Upper body</th>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i94 }}" name="i94"><input readonly type="checkbox" onclick="return false;" value="1" name="i94" {{ $report->i94 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i95 }}" name="i95"><input readonly type="checkbox" onclick="return false;" value="1" name="i95" {{ $report->i95 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i96 }}" name="i96"><input readonly type="checkbox" onclick="return false;" value="1" name="i96" {{ $report->i96 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;">ACTIVITIES</th>
                            <td><input readonly type="text" value="{{ $report->i97 }}" name="i97"></td>
                            <th>Dressing - Lower Body</th>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i98 }}" name="i98"><input readonly type="checkbox" onclick="return false;" value="1" name="i98" {{ $report->i98 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i99 }}" name="i99"><input readonly type="checkbox" onclick="return false;" value="1" name="i99" {{ $report->i99 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i100 }}" name="i100"><input readonly type="checkbox" onclick="return false;" value="1" name="i100" {{ $report->i100 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i101 }}" name="i101"></td>
                            <th>Toileting</th>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i102 }}" name="i102"><input readonly type="checkbox" onclick="return false;" value="1" name="i102" {{ $report->i102 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i103 }}" name="i103"><input readonly type="checkbox" onclick="return false;" value="1" name="i103" {{ $report->i103 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i104 }}" name="i104"><input readonly type="checkbox" onclick="return false;" value="1" name="i104" {{ $report->i104 ? 'checked' : '' }}></td>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i105 }}" name="i105"></td>
                            <th>Bathing</th>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i106 }}" name="i106"><input readonly type="checkbox" onclick="return false;" value="1" name="i106" {{ $report->i106 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i107 }}" name="i107"><input readonly type="checkbox" onclick="return false;" value="1" name="i107" {{ $report->i107 ? 'checked' : '' }}></td>
                            <td class="t-center"><input readonly type="hidden" value="{{ $report->i108 }}" name="i108"><input readonly type="checkbox" onclick="return false;" value="1" name="i108" {{ $report->i108 ? 'checked' : '' }}></td>
                            <td></td>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i109 }}" name="i109"></td>
                            <th style="border: none;">ASSISTED</th>
                            <td><input readonly type="text" value="{{ $report->i110 }}" name="i110"></td>
                            <th>Without assisted devices</th>
                            <input readonly type="hidden" value="{{ $report->i111 }}" name="i111">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i111" {{ $report->i111 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i112 }}" name="i112">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i112" {{ $report->i112 ? 'checked' : '' }}>&nbsp;Good</th>
                            <input readonly type="hidden" value="{{ $report->i113 }}" name="i113">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i113" {{ $report->i113 ? 'checked' : '' }}>&nbsp;Bad</th>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;">DEVICES</th>
                            <td><input readonly type="text" value="{{ $report->i114 }}" name="i114"></td>
                            <th>One crutch</th>
                            <input readonly type="hidden" value="{{ $report->i115 }}" name="i115">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i115" {{ $report->i115 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i116 }}" name="i116">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i116" {{ $report->i116 ? 'checked' : '' }}>&nbsp;Good</th>
                            <input readonly type="hidden" value="{{ $report->i117 }}" name="i117">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i117" {{ $report->i117 ? 'checked' : '' }}>&nbsp;Bad</th>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i118 }}" name="i118"></td>
                            <th>Pair of crutches</th>
                            <input readonly type="hidden" value="{{ $report->i119 }}" name="i119">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i119" {{ $report->i119 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i120 }}" name="i120">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i120" {{ $report->i120 ? 'checked' : '' }}>&nbsp;Good</th>
                            <input readonly type="hidden" value="{{ $report->i121 }}" name="i121">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i121" {{ $report->i121 ? 'checked' : '' }}>&nbsp;Bad</th>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i122 }}" name="i122"></td>
                            <th>Walking frame</th>
                            <input readonly type="hidden" value="{{ $report->i123 }}" name="i123">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i123" {{ $report->i123 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i124 }}" name="i124">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i124" {{ $report->i124 ? 'checked' : '' }}>&nbsp;Good</th>
                            <input readonly type="hidden" value="{{ $report->i125 }}" name="i125">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i125" {{ $report->i125 ? 'checked' : '' }}>&nbsp;Bad</th>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i126 }}" name="i126"></td>
                            <th>Wheelchair</th>
                            <input readonly type="hidden" value="{{ $report->i127 }}" name="i127">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i127" {{ $report->i127 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i128 }}" name="i128">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i128" {{ $report->i128 ? 'checked' : '' }}>&nbsp;Good</th>
                            <input readonly type="hidden" value="{{ $report->i129 }}" name="i129">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i129" {{ $report->i129 ? 'checked' : '' }}>&nbsp;Bad</th>
                            <td></td>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i130 }}" name="i130"></td>
                            <th>Orthoses right side</th>
                            <input readonly type="hidden" value="{{ $report->i131 }}" name="i131">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i131" {{ $report->i131 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i132 }}" name="i132">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i132" {{ $report->i132 ? 'checked' : '' }}>&nbsp;Good</th>
                            <input readonly type="hidden" value="{{ $report->i133 }}" name="i133">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i133" {{ $report->i133 ? 'checked' : '' }}>&nbsp;Bad</th>
                            <th>
                                <input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i134" {{ $report->i134 ? 'checked' : '' }}>&nbsp;FO
                                <input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i135" {{ $report->i135 ? 'checked' : '' }}>&nbsp;AFO
                                <input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i136" {{ $report->i136 ? 'checked' : '' }}>&nbsp;KAFO
                                <input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i137" {{ $report->i137 ? 'checked' : '' }}>&nbsp;HKAFO
                                <input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i138" {{ $report->i138 ? 'checked' : '' }}>&nbsp;Shoe raise
                            </th>
                        </tr>
                        <tr>
                            <th style="border: none;"></th>
                            <td><input readonly type="text" value="{{ $report->i139 }}" name="i139"></td>
                            <th>Orthosis left side</th>
                            <input readonly type="hidden" value="{{ $report->i140 }}" name="i140">
                            <td class="t-center"><input readonly type="checkbox" onclick="return false;" value="1" name="i140" {{ $report->i140 ? 'checked' : '' }}></td>
                            <input readonly type="hidden" value="{{ $report->i141 }}" name="i141">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i141" {{ $report->i141 ? 'checked' : '' }}>&nbsp;Good</th>
                            <input readonly type="hidden" value="{{ $report->i142 }}" name="i142">
                            <th class="t-center"><input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i142" {{ $report->i142 ? 'checked' : '' }}>&nbsp;Bad</th>
                            <th>
                                <input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i143" {{ $report->i143 ? 'checked' : '' }}>&nbsp;FO
                                <input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i144" {{ $report->i144 ? 'checked' : '' }}>&nbsp;AFO
                                <input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i145" {{ $report->i145 ? 'checked' : '' }}>&nbsp;KAFO
                                <input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i146" {{ $report->i146 ? 'checked' : '' }}>&nbsp;HKAFO
                                <input readonly class="keda" type="checkbox" onclick="return false;" value="1" name="i147" {{ $report->i147 ? 'checked' : '' }}>&nbsp;Shoe raise
                            </th>
                        </tr>

                    </tbody>
                </table>
            </div>
            <div class="border size-fit p-fit">
                <h3>Treatment Plan of Care:</h3>
                <div style="margin-left: 4rem;">
                    <h4>Problems List:</h4>
                    <ul>
                        @foreach ($report->problems_list ?? [] as $index => $problem)
                        <li>{{ $index + 1 }}- {{ $problem }}</li>
                        @endforeach
                    </ul>

                    <div style="display: flex; gap: 2rem; margin-top: 1rem;">
                        <div>
                            <h4>Short Term Goals:</h4>
                            <ul>
                                @foreach ($report->short_term_goals ?? [] as $index => $goal)
                                <li>{{ $index + 1 }}- {{ $goal }}</li>
                                @endforeach
                            </ul>
                        </div>
                        <div>
                            <h4>Long Term Goals:</h4>
                            <ul>
                                @foreach ($report->long_term_goals ?? [] as $index => $goal)
                                <li>{{ $index + 1 }}- {{ $goal }}</li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
                </div>
            </div>



            <div class="border size-fit p-fit">
                <h3>Re Assessment Plan of care After 6 sessions: </h3>
                <div style="margin-left: 4rem;">
                    <h4>Problems List:</h4>
                    <ul>
                        @foreach ($report->problems_list_6 ?? [] as $index => $problem)
                        <li>{{ $index + 1 }}- {{ $problem }}</li>
                        @endforeach
                    </ul>

                    <div style="display: flex; gap: 2rem; margin-top: 1rem;">
                        <div>
                            <h4>Short Term Goals:</h4>
                            <ul>
                                @foreach ($report->short_term_goals_6 ?? [] as $index => $goal)
                                <li>{{ $index + 1 }}- {{ $goal }}</li>
                                @endforeach
                            </ul>
                        </div>
                        <div>
                            <h4>Long Term Goals:</h4>
                            <ul>
                                @foreach ($report->long_term_goals_6 ?? [] as $index => $goal)
                                <li>{{ $index + 1 }}- {{ $goal }}</li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
                </div>
            </div>


            <div class="border size-fit p-fit">
                <h3>Re Assessment Plan of care After 12 sessions: </h3>
                <div style="margin-left: 4rem;">
                    <h4>Problems List:</h4>
                    <ul>
                        @foreach ($report->problems_list_12 ?? [] as $index => $problem)
                        <li>{{ $index + 1 }}- {{ $problem }}</li>
                        @endforeach
                    </ul>

                    <div style="display: flex; gap: 2rem; margin-top: 1rem;">
                        <div>
                            <h4>Short Term Goals:</h4>
                            <ul>
                                @foreach ($report->short_term_goals_12 ?? [] as $index => $goal)
                                <li>{{ $index + 1 }}- {{ $goal }}</li>
                                @endforeach
                            </ul>
                        </div>
                        <div>
                            <h4>Long Term Goals:</h4>
                            <ul>
                                @foreach ($report->long_term_goals_12 ?? [] as $index => $goal)
                                <li>{{ $index + 1 }}- {{ $goal }}</li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
                </div>
            </div>


        </body>

</html>
<div class='conta'>
    <a
        href="{{ route('physiotherapy_assessment.edit' , $report->id) }}" class="dko">
        تعديل
    </a>





    <div class='dko' name='btn4' onclick="printPage()">
        طباعة


    </div>
    <script>
        function printPage() {
            // تعيين الخصائص للطباعة
            var printOptions = {
                scale: 0.8, // SCALE 80%
                background: {
                    graphic: 'https://example.com/background-image.jpg', // background-graphic
                    display: 'print'
                }
            };

            // تنفيذ طباعة الصفحة مع الإعدادات
            window.print(printOptions);
        }
    </script>
    <style>
        @media print {
            .bcont {
                display: none !important;
            }

            aside {
                display: none !important;
            }

            header {
                display: none !important;
            }

            .inmain {
                width: 95% !important;
            }

            .container {
                width: 100% !important;
            }

            button {
                display: none;
            }

            /* تعيين الصفحة للطباعة في وضع landscape */
            @page {
                size: landscape;
            }
        }
    </style>



</div>
</form>
</body>
<style>
    .keda {

        color: red;
        /* border-radius: 50%; */
        box-shadow: 0 0 3px darkgreen;
        height: 60px;
        /* font-size: 22px; */
        width: 1em;
        height: 1rem;
        accent-color: green;
        -webkit-appearance: none;
        -moz-appearance: none;
        appearance: none;
        width: 20px;
        height: 20px;
        border: 1px dashed black;
        border-radius: 50%;
        outline: none;
        cursor: pointer;

    }

    .keda:checked {
        background: linear-gradient(0.25turn, #66AA3E, #92C641, #66AA3E);
    }
</style>

</html>
<style>
    .conta {
        margin-top: 30px;
        display: flex;
        justify-content: center;
        width: block;
    }

    button {
        margin: 20px;
        width: 15%;
        padding: 10px;
        border: none;
        border-radius: 5px;
        cursor: pointer;
        background: linear-gradient(0.25turn, #66AA3E, #92C641, #66AA3E);
        color: black;
        font-family: 'Droid Arabic Kufi', Arial, sans-serif;
        font-weight: bold;
        font-family: cairo;
    }






    header {
        position: fixed !important;
        width: 100% !important;
        top: 0px;
    }

    header,
    footer {
        text-align: center;
    }

    header,
    footer,
    aside,
    main {
        background-color: #f0f0f0;
    }

    header {
        height: 130px;
        background-color: #f0f0f0;
        display: flex !important;
        z-index: 100000;
        position: relative;
        box-shadow: 0 0 10px black;
        font-family: cairo;
    }

    .v {
        filter: unset !important;
    }

    .b1 {
        font-size: 18px;
        width: 100px;
        height: 130px;
        color: black;
        vertical-align: middle;
        clear: both;
        text-align: center;
        display: flex;
        justify-content: center;
        align-items: center;
        transition: 0.4s;
        -webkit-transition-duration: .4s;
        transition-duration: 0.4s;
        font-family: cairo;
    }

    .v {
        font-weight: bold;
        background-color: #f0f0f0;
        filter: drop-shadow(2px -10px 6px black);
    }

    .logoi {
        filter: unset !important;
    }

    .logoi {
        height: 90%;
        filter: drop-shadow(0px 0px 4px black);
    }

    img {
        width: 100%;
        height: auto;
    }

    user agent stylesheet img {
        overflow-clip-margin: content-box;
        overflow: clip;
    }

    .logo {
        justify-content: center;
        display: flex;
        left: calc(50% - 60px);
        position: absolute;
        height: 140px;
        width: 140px;
        z-index: 1;
    }

    body {
        margin-top: 20px;
        background-color: #f0f0f0;
    }
</style>