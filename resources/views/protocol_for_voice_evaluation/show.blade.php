<link rel="stylesheet" href="vv0all.css">

<style>
    @media print {
        .bcont {
            display: none !important;
        }

        aside {
            display: none !important;
        }

        header {
            display: none !important;
        }

        .inmain {
            width: 95% !important;
        }

        .container {
            width: 100% !important;
        }

        button {
            display: none;
        }

        /* تعيين الصفحة للطباعة في وضع landscape */
        @page {
            size: landscape;
        }
    }

    h1 {
        font-family: cairo;
        text-align: center;



        text-align: center;
    }

    .dko {

        margin: 20px;
        width: 15%;
        padding: 10px;
        border: none;
        border-radius: 5px;
        cursor: pointer;
        background: linear-gradient(0.25turn, #66AA3E, #92C641, #66AA3E);
        color: black;
        font-family: 'Droid Arabic Kufi', Arial, sans-serif;
        font-weight: bold;
        font-family: cairo;
        text-align: center;
    }
</style>
<link href="https://fonts.googleapis.com/css?family=Cairo" rel="stylesheet">


<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <style>
        * {
            font-family: Sans-serif;
        }

        .border {
            border: 2px solid black;
        }

        label {
            font-weight: bolder;
        }

        .row {
            display: flex;
            gap: 1rem;
            margin-bottom: 1rem;
        }

        .flex {
            display: flex;
        }

        .w-full {
            width: 100%;
        }

        .p-0 {
            padding: 0;
        }

        .font-bold {
            font-weight: bolder;
        }

        .font-w-none {
            font-weight: normal;
        }

        .size-fit {
            min-width: 820px;
            max-width: 1060px;
        }

        .p-fit {
            padding: 2rem 3rem;
        }

        table {
            border-collapse: collapse;
            width: 100%;
        }

        th,
        td {
            border: 1px solid black;
            padding: 8px;
            text-align: left;
        }

        .t-center {
            text-align: center;
        }

        .bg-grey {
            background-color: rgb(238, 238, 238);
        }

        .t-grey {
            color: #a7a4a4;
        }

        .RangeOfMotionTable th,
        td {
            padding: 0 4px;
        }

        .b-none {
            border: none;
        }

        .input readonly -none {
            border: none;
            outline: none;
        }

        .h-w-full {
            height: 100%;
            width: 100%;
        }

        .t-w-fit {
            width: 96%;
        }

        .bg-yellow {
            background-color: rgb(249, 249, 147);
        }

        .h-w-full {
            height: 100%;
            width: 100%;
            background-color: #f0f0f0;
        }

        .bg-yellow {
            background-color: rgb(249, 249, 147);
        }

        th,
        td {
            padding: 0 0px;
            height: 30px;
            background-color: #f0f0f0 !important;
            height: 30px;
        }

        .b-none {
            border: none;
            color: darkgreen;
        }

        * {
            font-family: Sans-serif;
        }

        .Subjective_Assessment_con {
            padding: 0 .5rem 1rem 1rem;
        }

        .border {
            border: 2px solid black;
        }

        .py-lg {
            padding-top: 2rem;
            padding-bottom: 2rem;
        }

        .px-md {
            padding-left: 1rem;
            padding-right: 1rem;
        }

        label {
            font-weight: bolder;

        }

        .row {
            flex-direction: column;
            display: flex;
            gap: 1rem;
            margin-bottom: 1rem;
        }

        .flex {
            display: flex;
        }

        .w-half {
            width: 50%;
        }

        .w-full {
            width: 100%;
        }



        table {
            border-collapse: collapse;
            padding: 4px;
            width: 100%;
        }

        .p-0 {
            padding: 0;
        }

        .font-bold {
            font-weight: bolder;
        }
    </style>
</head>

<!-- alert -->
@if(session('success'))
<div class="alert alert-success">
    {{ session('success') }}
</div>
@endif
@if(session('error'))
<div class="alert alert-danger">
    {{ session('error') }}

</div>
@endif


<form style="margin-top: 50px;">


    <body style="display: flex; flex-direction: column; font-weight: .5rem; align-items: center; gap: 2rem;">
        <div style="min-width: 820px; max-width: 1060px;font-size: 14px;" class="border Subjective_Assessment_con">
            <h3 align="center"><u>PROTOCOL FOR VOICE EVALUATION</u></h3>
            <div class="row" style="gap: .2rem;">
                <div>
                    <span>Referral from :</span>&nbsp;
                    <input readonly type="text" value="{{ old('referral_from', $report->referral_from) }}" name="referral_from">
                </div>
                <div>
                    <span>Date :</span>&nbsp;
                    <input readonly type="date" value="{{ old('evaluation_date', $report->evaluation_date) }}" name="evaluation_date">
                </div>
                <div>
                    <span>Cause of referral:</span>&nbsp;
                    <input readonly type="text" style="width: 40%;" value="{{ old('cause_of_referral', $report->cause_of_referral) }}" name="cause_of_referral">
                </div>
            </div>
            <div class="row" style="gap: .2rem; margin-left: 2rem;">
                <label style="margin-left: -1rem;">1)Personal data</label>
                <div>
                    <span>Name:</span>&nbsp;
                    <input readonly type="text" value="{{ old('name', $report->name) }}" name="name">
                </div>
                <div>
                    <span>Age:</span>&nbsp;
                    <input readonly type="text" style="width: 100px;" value="{{ old('age', $report->age) }}" name="age">
                </div>
                <div>
                    <span>Sex:</span>&nbsp;
                    <input readonly type="text" style="width: 100px;" value="{{ old('sex', $report->sex) }}" name="sex">
                </div>
                <div>
                    <span>Residence:</span>&nbsp;
                    <input readonly type="text" value="{{ old('residence', $report->residence) }}" name="residence">
                </div>
                <div>
                    <span>Education:</span>&nbsp;
                    <input readonly type="text" value="{{ old('education', $report->education) }}" name="education">
                </div>
                <div>
                    <span>Occupation:</span>&nbsp;
                    <input readonly type="text" value="{{ old('occupation', $report->occupation) }}" name="occupation">
                </div>
                <div>
                    <span>Material state:</span>&nbsp;
                    <input readonly type="text" value="{{ old('material_state', $report->material_state) }}" name="material_state">
                </div>
                <div>
                    <span>Children no:</span>&nbsp;
                    <input readonly style="width: 100px;" type="text" value="{{ old('children_no', $report->children_no) }}" name="children_no">
                </div>
            </div>
            <label>2) COMPLAINT & ANALYSIS OF SYMPTOMS:</label> <br>
            <textarea cols="100" name="complaint_analysis">{{ old('complaint_analysis', $report->complaint_analysis) }}</textarea>

            <div class="row" style="gap: .2rem;margin-left: 2rem;">
                <div>
                    <span>Duration:</span>&nbsp;
                    <input readonly type="text" value="{{ old('duration', $report->duration) }}" name="duration">
                </div>
                <div>
                    <span>Onset:</span>&nbsp;
                    <input readonly type="text" value="{{ old('onset', $report->onset) }}" name="onset">
                </div>
                <div>
                    <span>Following:</span>&nbsp;
                    <input readonly type="text" value="{{ old('following', $report->following) }}" name="following">
                </div>
                <div>
                    <span>Course:</span>&nbsp;
                    <input readonly type="text" value="{{ old('course', $report->course) }}" name="course">
                </div>
            </div>
            <label><u>Phonasthenic symptoms</u></label>
            <ul>
                <li>
                    <span>Frequent throat clearing</span>&nbsp;
                    <!-- hidden input readonly  for storing the value -->
                    <input readonly type="hidden" name="frequent_throat_clearing" value="0">
                    <input readonly type="checkbox" onclick="return false;" name="frequent_throat_clearing" value="1" {{ old('frequent_throat_clearing', $report->frequent_throat_clearing) ? 'checked' : '' }}>
                </li>
                <li>
                    <span> Sticky Throat mucous</span>&nbsp;
                    <input readonly type="hidden" name="sticky_throat_mucous" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="sticky_throat_mucous" {{ old('sticky_throat_mucous', $report->sticky_throat_mucous) ? 'checked' : '' }}>
                </li>
                <li>
                    <span> Tenderness</span>&nbsp;
                    <input readonly type="hidden" name="tenderness" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="tenderness" {{ old('tenderness', $report->tenderness) ? 'checked' : '' }}>
                </li>
                <li>
                    <span>dryness</span>&nbsp;
                    <input readonly type="hidden" name="dryness" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="dryness" {{ old('dryness', $report->dryness) ? 'checked' : '' }}>
                </li>
            </ul>
            <label><u>Patient's rating of the severity:</u></label>
            <ul>
                <li>
                    <span>0</span>&nbsp;
                    <input readonly type="hidden" name="severity_rating_zero" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="severity_rating_zero" {{ old('severity_rating_zero', $report->severity_rating_zero) ? 'checked' : '' }}>
                </li>
                <li>
                    <span>1</span>&nbsp;
                    <input readonly type="hidden" name="severity_rating_one" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="severity_rating_one" {{ old('severity_rating_one', $report->severity_rating_one) ? 'checked' : '' }}>
                </li>
                <li>
                    <span> 2</span>&nbsp;
                    <input readonly type="hidden" name="severity_rating_two" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="severity_rating_two" {{ old('severity_rating_two', $report->severity_rating_two) ? 'checked' : '' }}>
                </li>
                <li>
                    <span>3</span>&nbsp;
                    <input readonly type="hidden" name="severity_rating_three" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="severity_rating_three" {{ old('severity_rating_three', $report->severity_rating_three) ? 'checked' : '' }}>
                </li>
                <li>
                    <span>4</span>&nbsp;
                    <input readonly type="hidden" name="severity_rating_four" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="severity_rating_four" {{ old('severity_rating_four', $report->severity_rating_four) ? 'checked' : '' }}>
                </li>
            </ul>
            <label><u>Llistener’s reaction :</u></label>
            <ul>
                <li>
                    <span>+ ve</span>&nbsp;
                    <input readonly type="hidden" name="listeners_reaction_postive" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="listeners_reaction_postive" {{ old('listeners_reaction_postive', $report->listeners_reaction_postive) ? 'checked' : '' }}>
                </li>
                <li>
                    <span>- ve</span>&nbsp;
                    <input readonly type="hidden" name="listeners_reaction_negative" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="listeners_reaction_negative" {{ old('listeners_reaction_negative', $report->listeners_reaction_negative) ? 'checked' : '' }}>
                </li>
            </ul>
            <label><u>Vocal demand:</u></label>
            <ul>
                <li>
                    <span>high</span>&nbsp;
                    <input readonly type="hidden" name="vocal_demand_high" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="vocal_demand_high" {{ old('vocal_demand_high', $report->vocal_demand_high) ? 'checked' : '' }}>
                </li>
                <li>
                    <span>low</span>&nbsp;
                    <input readonly type="hidden" name="vocal_demand_low" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="vocal_demand_low" {{ old('vocal_demand_low', $report->vocal_demand_low) ? 'checked' : '' }}>
                </li>
            </ul>
            <label><u>Job environment:</u></label>
            <ul class="row" style="gap: .1rem;">
                <li>
                    <span> Noise:</span>&nbsp;
                    <input readonly style="width: 40%;" type="text" value="{{ old('noise', $report->noise) }}" name="noise">
                </li>
                <li>
                    <span>Relative humidity: </span>&nbsp;
                    <input readonly style="width: 40%;" type="text" value="{{ old('relative_humidity', $report->relative_humidity) }}" name="relative_humidity">
                </li>
                <li>
                    <span> temperature: </span>&nbsp;
                    <input readonly style="width: 40%;" type="text" value="{{ old('temperature', $report->temperature) }}" name="temperature">
                </li>
                <li>
                    <span>Irritants: </span>&nbsp;
                    <input readonly style="width: 40%;" type="text" value="{{ old('irritants', $report->irritants) }}" name="irritants">
                </li>
            </ul>
            <label><u>Smoking:</u></label>
            <ul class="row" style="gap: .1rem;">
                <li>
                    <span> quantity</span>&nbsp;
                    <input readonly style="width: 40%;" type="text" value="{{ old('smoking_quantity', $report->smoking_quantity) }}" name="smoking_quantity">
                </li>
                <li>
                    <span>duration </span>&nbsp;
                    <input readonly style="width: 40%;" type="text" value="{{ old('smoking_duration', $report->smoking_duration) }}" name="smoking_duration">
                </li>
                <li>
                    <span> temperature: </span>&nbsp;
                    <input readonly style="width: 40%;" type="text" value="{{ old('smoking_temperature', $report->smoking_temperature) }}" name="smoking_temperature">
                </li>
                <li>
                    <span> past history </span>&nbsp;
                    <input readonly style="width: 40%;" type="text" value="{{ old('smoking_past_history', $report->smoking_past_history) }}" name="smoking_past_history">
                </li>
            </ul>
        </div>
        <div style="min-width: 820px; max-width: 1060px;font-size: 14px;" class="border Subjective_Assessment_con">
            <label><u>Spirits :</u></label>
            <ul>
                <li>
                    <span>+ ve</span>&nbsp;
                    <!-- hidden input readonly  for storing the value -->
                    <input readonly type="hidden" name="spirits_postive" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="spirits_postive" {{ old('spirits_postive', $report->spirits_postive) ? 'checked' : '' }}>
                </li>
                <li>
                    <span>- ve</span>&nbsp;
                    <input readonly type="hidden" name="spirits_negative" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="spirits_negative" {{ old('spirits_negative', $report->spirits_negative) ? 'checked' : '' }}>
                </li>
            </ul>
            <div>
                <span>Duration</span>&nbsp;
                <textarea cols="100" name="spirits_duration">{{ old('spirits_duration', $report->spirits_duration) }}</textarea>
            </div>
            <label><u>Temperament:</u></label>
            <ul>
                <li>
                    <span>quiet</span>&nbsp;
                    <input readonly type="hidden" value="1" name="i41">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="i41" {{ old('i41', $report->i41) ? 'checked' : '' }}>
                </li>
                <li>
                    <span>tense</span>&nbsp;
                    <input readonly type="hidden" value="1" name="i42">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="i42" {{ old('i42', $report->i42) ? 'checked' : '' }}>
                </li>
            </ul>
            <label><u>Emotional Stress: </u></label>
            <ul>
                <li>
                    <span>+ ve</span>&nbsp;
                    <input readonly type="hidden" name="emotional_stress_postive" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="emotional_stress_postive" {{ old('emotional_stress_postive', $report->emotional_stress_postive) ? 'checked' : '' }}>
                </li>
                <li>
                    <span>- ve</span>&nbsp;
                    <input readonly type="hidden" name="emotional_stress_negative" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="emotional_stress_negative" {{ old('emotional_stress_negative', $report->emotional_stress_negative) ? 'checked' : '' }}>
                </li>
            </ul>
            <label><u>Allergic Tendencies </u></label>
            <ul>
                <li>
                    <span>+ ve</span>&nbsp;
                    <input readonly type="hidden" name="allergic_tendencies_postive" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="allergic_tendencies_postive" {{ old('allergic_tendencies_postive', $report->allergic_tendencies_postive) ? 'checked' : '' }}>
                </li>
                <li>
                    <span>- ve</span>&nbsp;
                    <input readonly type="hidden" name="allergic_tendencies_negative" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="allergic_tendencies_negative" {{ old('allergic_tendencies_negative', $report->allergic_tendencies_negative) ? 'checked' : '' }}>
                </li>
            </ul>
            <div class="row" style="gap: .2rem;">
                <div>
                    <span>Repeated U.R.T infections</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('urt_infections', $report->urt_infections) }}" name="urt_infections">
                </div>
                <div>
                    <span>Chronic cough & chest diseases</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('chronic_cough', $report->chronic_cough) }}" name="chronic_cough">
                </div>
                <div>
                    <span>Breathing:</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('breathing', $report->breathing) }}" name="breathing">
                </div>
                <div>
                    <span>Chewing & swallowing:</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('chewing_swallowing', $report->chewing_swallowing) }}" name="chewing_swallowing">
                </div>
                <div>
                    <span>Hyperacidity& Reflux :</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('hyperacidity_reflux', $report->hyperacidity_reflux) }}" name="hyperacidity_reflux">
                </div>
            </div>
            <ul>
                <li>
                    <span>+ ve</span>&nbsp;
                    <input readonly type="hidden" name="positive" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="positive" {{ old('positive', $report->positive) ? 'checked' : '' }}>
                </li>
                <li>
                    <span>- ve</span>&nbsp;
                    <input readonly type="hidden" name="negative" value="0">
                    <input readonly type="checkbox" onclick="return false;" value="1" name="negative" {{ old('negative', $report->negative) ? 'checked' : '' }}>
                </li>
            </ul>
            <div class="row" style="gap: .2rem;">
                <div>
                    <span>Medicaments</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('medicaments', $report->medicaments) }}" name="medicaments">
                </div>
                <div>
                    <span>Surgical intervention& trauma</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('surgical_intervention', $report->surgical_intervention) }}" name="surgical_intervention">
                </div>
                <div style="margin-left: 1rem;">
                    <label><u>3 )AUDITORY PERCEPTUAL ASSESSMENT (APA):</u></label>&nbsp;
                </div>
                <div style="margin-left: 1rem;">
                    <span>GRBAS</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('grbas', $report->grbas) }}" name="grbas">
                </div>
                <div>
                    <span>Pitch</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('pitch', $report->pitch) }}" name="pitch">
                </div>
            </div>
            <div class="row" style="gap: .2rem;">
                <span>Diplophonia :</span>
                <ul>
                    <li>
                        <span>Yes</span>&nbsp;
                        <input readonly type="hidden" name="diplophonia_yes" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="diplophonia_yes" {{ old('diplophonia_yes', $report->diplophonia_yes) ? 'checked' : '' }}>
                    </li>
                    <li>
                        <span>No</span>&nbsp;
                        <input readonly type="hidden" name="diplophonia_no" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="diplophonia_no" {{ old('diplophonia_no', $report->diplophonia_no) ? 'checked' : '' }}>
                    </li>
                </ul>
                <span>Register : </span>
                <ul>
                    <li>
                        <span>Habitual</span>&nbsp;
                        <input readonly type="hidden" name="register_0" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="register_1" {{ old('register_1', $report->register_1) ? 'checked' : '' }}>
                    </li>
                    <li>
                        <span>modal</span>&nbsp;
                        <input readonly type="hidden" name="register_2" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="register_2" {{ old('register_2', $report->register_2) ? 'checked' : '' }}>
                    </li>
                    <li>
                        <span>falsetto</span>&nbsp;
                        <input readonly type="hidden" name="register_3" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="register_3" {{ old('register_3', $report->register_3) ? 'checked' : '' }}>
                    </li>
                </ul>
                <span>Tendency of vocal fry at the end of phrase:</span>
                <ul>
                    <li>
                        <span>Yes</span>&nbsp;
                        <input readonly type="hidden" name="vocal_fry_1" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="vocal_fry_1" {{ old('vocal_fry_1', $report->vocal_fry_1) ? 'checked' : '' }}>
                    </li>
                    <li>
                        <span>No</span>&nbsp;
                        <input readonly type="hidden" name="vocal_fry_2" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="vocal_fry_2" {{ old('vocal_fry_2', $report->vocal_fry_2) ? 'checked' : '' }}>
                    </li>
                </ul>
                <span>Register break:</span>
                <ul>
                    <li>
                        <span>Yes</span>&nbsp;
                        <input readonly type="hidden" name="register_break_1" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="register_break_1" {{ old('register_break_1', $report->register_break_1) ? 'checked' : '' }}>
                    </li>
                    <li>
                        <span>No</span>&nbsp;
                        <input readonly type="hidden" name="register_break_2" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="register_break_2" {{ old('register_break_2', $report->register_break_2) ? 'checked' : '' }}>
                    </li>
                </ul>
                <span>Loudness:</span>
                <ul>
                    <li>
                        <span>Excessive</span>&nbsp;
                        <input readonly type="hidden" name="loudness_ex" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="loudness_ex" {{ old('loudness_ex', $report->loudness_ex) ? 'checked' : '' }}>
                    </li>
                    <li>
                        <span>Soft</span>&nbsp;
                        <input readonly type="hidden" name="loudness_soft" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="loudness_soft" {{ old('loudness_soft', $report->loudness_soft) ? 'checked' : '' }}>
                    </li>
                </ul>
                <div>
                    <span>Glottal attack:</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('glottal_attack', $report->glottal_attack) }}" name="glottal_attack">
                </div>
                <label><u>Associated laryngeal functions :</u></label>
                <div>
                    <span>Cough</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('cough', $report->cough) }}" name="cough">
                </div>
                <div>
                    <span>whisper</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('whisper', $report->whisper) }}" name="whisper">
                </div>
                <div>
                    <span>laughter</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('laughter', $report->laughter) }}" name="laughter">
                </div>
                <div>
                    <span>Oral cavity:</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('oral_cavity', $report->oral_cavity) }}" name="oral_cavity">
                </div>
            </div>
        </div>
        <div style="min-width: 820px; max-width: 1060px;font-size: 14px;" class="border Subjective_Assessment_con">
            <label><u>Pharynx :</u></label>
            <div class="row" style="gap: .2rem;">
                <span>Tonsils :</span>
                <ul>
                    <li>
                        <span>+ ve</span>&nbsp;
                        <input readonly type="hidden" name="tonsils_postive" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="tonsils_postive" {{ old('tonsils_postive', $report->tonsils_postive) ? 'checked' : '' }}>
                    </li>
                    <li>
                        <span>- ve</span>&nbsp;
                        <input readonly type="hidden" name="tonsils_negative" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="tonsils_negative" {{ old('tonsils_negative', $report->tonsils_negative) ? 'checked' : '' }}>
                    </li>
                </ul>
                <span>Post nasal discharge:</span>
                <ul>
                    <li>
                        <span>+ ve</span>&nbsp;
                        <input readonly type="hidden" name="post_nasal_discharge_postive" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="post_nasal_discharge_postive" {{ old('post_nasal_discharge_postive', $report->post_nasal_discharge_postive) ? 'checked' : '' }}>
                    </li>
                    <li>
                        <span>- ve</span>&nbsp;
                        <input readonly type="hidden" name="post_nasal_discharge_negative" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="post_nasal_discharge_negative" {{ old('post_nasal_discharge_negative', $report->post_nasal_discharge_negative) ? 'checked' : '' }}>
                    </li>
                </ul>
                <span>Laryngeal skeleton:</span>
                <ul>
                    <li>
                        Normal configuration&nbsp;
                        <input readonly type="hidden" name="laryngeal_skeleton_normal" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="laryngeal_skeleton_normal" {{ old('laryngeal_skeleton_normal', $report->laryngeal_skeleton_normal) ? 'checked' : '' }}>
                    </li>
                    <li>
                        Fractures:&nbsp;
                        <input readonly type="hidden" name="fractures" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="fractures" {{ old('fractures', $report->fractures) ? 'checked' : '' }}><br>
                        site&nbsp;
                        <input readonly type="text" style="width: 60%;" value="{{ old('fracture_site', $report->fracture_site) }}" name="fracture_site"><br>
                        type&nbsp;
                        <input readonly type="text" style="width: 60%;" value="{{ old('fracture_type', $report->fracture_type) }}" name="fracture_type">
                    </li>
                    <li>
                        other abnormalities&nbsp;
                        <input readonly type="hidden" name="abnormalities" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="abnormalities" {{ old('abnormalities', $report->abnormalities) ? 'checked' : '' }}>
                    </li>
                </ul>
                <div>
                    <span>Laryngeal click</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('laryngeal_click', $report->laryngeal_click) }}" name="laryngeal_click">
                </div>
                <div>
                    <span>Laryngeal position</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('laryngeal_position', $report->laryngeal_position) }}" name="laryngeal_position">
                </div>
                <div>
                    <span>Cervical veins</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('cervical_veins', $report->cervical_veins) }}" name="cervical_veins">
                </div>
                <label>Neck scars :</label>
                <ul>
                    <li>
                        Yes :&nbsp;
                        <input readonly type="hidden" name="neck_scars_yes" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="neck_scars_yes" {{ old('neck_scars_yes', $report->neck_scars_yes) ? 'checked' : '' }}><br>
                        type&nbsp;
                        <input readonly type="text" style="width: 60%;" value="{{ old('scar_type', $report->scar_type) }}" name="scar_type"><br>
                        site&nbsp;
                        <input readonly type="text" style="width: 60%;" value="{{ old('scar_site', $report->scar_site) }}" name="scar_site"> <br>
                        size&nbsp;
                        <input readonly type="text" style="width: 60%;" value="{{ old('scar_size', $report->scar_size) }}" name="scar_size">
                    </li>
                    <li>
                        No&nbsp;
                        <input readonly type="hidden" name="neck_scars_no" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="neck_scars_no" {{ old('neck_scars_no', $report->neck_scars_no) ? 'checked' : '' }}>
                    </li>
                </ul>
                <h4><u>INDIRECT LARYNGOSCOPY</u></h4>
                <label><u>VOCAL FOLDS:</u></label>
                <div>
                    <span>Color:</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('vocal_color', $report->vocal_color) }}" name="vocal_color">
                </div>
                <div>
                    <span>Luster:</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('vocal_luster', $report->vocal_luster) }}" name="vocal_luster">
                </div>
                <div>
                    <span>Transparency:</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('vocal_transparency', $report->vocal_transparency) }}" name="vocal_transparency">
                </div>
                <div>
                    <span>Vascular markings:</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('vascular_markings', $report->vascular_markings) }}" name="vascular_markings">
                </div>
                <div>
                    <span>Swelling:</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('swelling', $report->swelling) }}" name="swelling">
                </div>
                <ul>
                    <li>
                        <span>Edge</span>&nbsp;
                        <input readonly type="hidden" name="edge" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="edge" {{ old('edge', $report->edge) ? 'checked' : '' }}>
                    </li>
                    <li>
                        <span>Surface</span>&nbsp;
                        <input readonly type="hidden" name="surface" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="surface" {{ old('surface', $report->surface) ? 'checked' : '' }}>
                    </li>
                </ul>
                <span>Ulcers: </span>
                <ul>
                    <li>
                        <span>No</span>&nbsp;
                        <input readonly type="hidden" name="has_ulcers_1" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="has_ulcers_1" {{ old('has_ulcers_1', $report->has_ulcers_1) ? 'checked' : '' }}>
                    </li>
                    <li>
                        Yes :&nbsp;
                        <input readonly type="hidden" name="has_ulcers_2" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="has_ulcers_2" {{ old('has_ulcers_2', $report->has_ulcers_2) ? 'checked' : '' }}><br>
                        Site&nbsp;
                        <input readonly type="text" style="width: 60%;" value="{{ old('ulcer_site', $report->ulcer_site) }}" name="ulcer_site"><br>
                        size&nbsp;
                        <input readonly type="text" style="width: 60%;" value="{{ old('ulcer_size', $report->ulcer_size) }}" name="ulcer_size"> <br>
                        floor&nbsp;
                        <input readonly type="text" style="width: 60%;" value="{{ old('ulcer_floor', $report->ulcer_floor) }}" name="ulcer_floor"> <br>
                        edge&nbsp;
                        <input readonly type="text" style="width: 60%;" value="{{ old('ulcer_edge', $report->ulcer_edge) }}" name="ulcer_edge">
                    </li>
                </ul>
                <div>
                    <span>Girth</span>&nbsp;
                    <input readonly type="text" style="width: 50%" value="{{ old('girth', $report->girth) }}" name="girth">
                </div>
                <span>Glottis :</span>
                <div class="flex">
                    <div style="width: 65%;">
                        <ul>
                            <li>
                                <span>Symmetrical</span>&nbsp;
                                <input readonly type="hidden" name="glottis_symmetry" value="0">
                                <input readonly type="checkbox" onclick="return false;" value="1" name="glottis_symmetry" {{ old('glottis_symmetry', $report->glottis_symmetry) ? 'checked' : '' }}>
                            </li>
                            <li>
                                <span>Asymmetrical</span>&nbsp;
                                <input readonly type="hidden" name="glottis_asymmetry" value="0">
                                <input readonly type="checkbox" onclick="return false;" value="1" name="glottis_asymmetry" {{ old('glottis_asymmetry', $report->glottis_asymmetry) ? 'checked' : '' }}>
                            </li>
                        </ul>
                        <span>Deviation:</span>
                        <ul>
                            <li>
                                <span>No</span>&nbsp;
                                <input readonly type="hidden" name="has_deviation_1" value="0">
                                <input readonly type="checkbox" onclick="return false;" value="1" name="has_deviation_1" {{ old('has_deviation_1', $report->has_deviation_1) ? 'checked' : '' }}>
                            </li>
                            <li>
                                <span>Yes</span>&nbsp;
                                <input readonly type="hidden" name="has_deviation_2" value="0">
                                <input readonly type="checkbox" onclick="return false;" value="1" name="has_deviation_2" {{ old('has_deviation_2', $report->has_deviation_2) ? 'checked' : '' }}>
                            </li>
                        </ul>
                        <div>
                            <span>To</span>&nbsp;
                            <input readonly type="text" style="width: 90%" value="{{ old('deviation_to', $report->deviation_to) }}" name="deviation_to">
                        </div>
                        <span>Glottic gap</span>
                    </div>
                    <div>
                        <img src="data:image/png;base64,"
                            alt="Not found">
                    </div>
                </div>
                <ul>
                    <li>
                        <span>No</span>&nbsp;
                        <input readonly type="hidden" name="has_glottic_gap_1" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="has_glottic_gap_1" {{ old('has_glottic_gap_1', $report->has_glottic_gap_1) ? 'checked' : '' }}>
                    </li>
                    <li>
                        Yes :&nbsp;
                        <input readonly type="hidden" name="has_glottic_gap_2" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="has_glottic_gap_2"
                            {{ old('has_glottic_gap_2', $report->has_glottic_gap_2) ? 'checked' : '' }}>
                        ><br>
                        Site&nbsp;
                        <input readonly type="text" style="width: 60%;" value="{{old('glottic_gap_site', $report->glottic_gap_site)}}" name="glottic_gap_site"><br>
                        size&nbsp;
                        <input readonly type="text" style="width: 60%;" value="{{ old('glottic_gap_size', $report->glottic_gap_size) }}" name="glottic_gap_size">
                    </li>
                </ul>
                <span>Phonatory waste :</span>
                <ul>
                    <li>
                        <span>No</span>&nbsp;
                        <input readonly type="hidden" name="phonatory_waste_1" value="0">
                        <input readonly type="checkbox" onclick="return false;"
                            value="1" name="phonatory_waste_1" {{ old('phonatory_waste_1', $report->phonatory_waste_1) ? 'checked' : '' }}>
                    </li>
                    <li>
                        <span>Yes</span>&nbsp;
                        <input readonly type=" hidden" name="phonatory_waste_2" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="phonatory_waste_2"
                            {{ old('phonatory_waste_2', $report->phonatory_waste_2) ? 'checked' : '' }}>
                        >
                    </li>
                </ul>
            </div>
        </div>
        <div style="min-width: 820px; max-width: 1060px;font-size: 14px;" class="border Subjective_Assessment_con">
            <div class="row" style="gap: .2rem;">
                <div class="flex" style="margin-top: 1rem;">
                    <span class="flex"><span>Respiratory</span>&nbsp; <span>chink</span></span>&nbsp;
                    <input readonly type="text" style="width: 85%" value="{{ old('respiratory_chink', $report->respiratory_chink) }} " name=" respiratory_chink">
                </div>
                <span>Phase closure Predominate</span>
                <ul>
                    <li>
                        <span>Open</span>&nbsp;
                        <input readonly type="hidden" name="phase_closure_open" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="phase_closure_open"
                            {{ old('phase_closure_open', $report->phase_closure_open) ? 'checked' : '' }}>
                    </li>
                    <li>
                        <span>Close</span>&nbsp;
                        <input readonly type="hidden" name="phase_closure_closed" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="phase_closure_closed"
                            {{ old('phase_closure_closed', $report->phase_closure_closed) ? 'checked' : '' }}>
                    </li>
                </ul>
                <label><u>Glottic Wave</u></label>
                <span>Stroboscopic fixation :</span>
                <ul>
                    <li>
                        <span>No</span>&nbsp;
                        <input readonly type="hidden" name="stroboscopic_fixation_1" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="stroboscopic_fixation_1"
                            {{ old('stroboscopic_fixation_1', $report->stroboscopic_fixation_1) ? 'checked' : '' }}>
                    </li>
                    <li>
                        <span>Yes</span>&nbsp;
                        <input readonly type="hidden" name="stroboscopic_fixation_2" value="0">
                        <input readonly type="checkbox" onclick="return false;" value="1" name="stroboscopic_fixation_2"
                            {{ old('stroboscopic_fixation_2', $report->stroboscopic_fixation_2) ? 'checked' : '' }}>
                    </li>
                </ul>
                <div>
                    <span>Segment</span>&nbsp;
                    <input readonly type="text" style="width: 70%" value="{{ old('segment', $report->segment) }}" name=" segment">
                </div>
                <div>
                    <span>Additional morphological findings details :</span>&nbsp;
                    <input readonly type="text" style="width: 40%" value="{{ old('morphological_findings', $report->morphological_findings) }}" name="morphological_findings">
                </div>
                <label><u>Ventricular folds : </u></label>
                <div>
                    <span>Color</span>&nbsp;
                    <input readonly type="text" style="width: 70%" value="{{ old('ventricular_color', $report->ventricular_color) }}" name="ventricular_color">
                </div>
                <div>
                    <span>masses</span>&nbsp;
                    <input readonly type="text" style="width: 70%" value="{{ old('ventricular_masses', $report->ventricular_masses) }}" name="ventricular_masses">
                </div>
                <div>
                    <span>Girth</span>&nbsp;
                    <input readonly type="text" style="width: 70%" value="   {{ old('ventricular_girth', $report->ventricular_girth) }}"
                        name="ventricular_girth">
                </div>
                <div>
                    <span>Position at phonation</span>&nbsp;
                    <input readonly type="text" style="width: 70%" value="  {{ old('phonation_position', $report->phonation_position) }}"
                        name="phonation_position">
                </div>
                <label><u>ADDITIONAL INSTRUMENTAL MEASURE</u></label>
                <div>
                    <span>ACOUSTIC ANALYSIS :</span>&nbsp;
                    <input readonly type="text" style="width: 70%" value="
                    {{ old('acoustic_analysis', $report->acoustic_analysis) }} " name=" acoustic_analysis">
                </div>
                <div>
                    <span>RADIOLOGICAL STUDIES</span>&nbsp;
                    <input readonly type="text" style="width: 70%" value="   {{ old('radiological_studies', $report->radiological_studies) }}
                    " name="radiological_studies">
                </div>
            </div>
        </div>

        <div class='conta'>
            <a href=" {{ route('protocol_for_voice_evaluation.edit', $report->id) }}" class="dko" name='btn3'>
                تعديل
            </a>




            <div class='dko' name='btn4' onclick="printPage()">
                طباعة


            </div>
            <script>
                function printPage() {
                    // تعيين الخصائص للطباعة
                    var printOptions = {
                        scale: 0.8, // SCALE 80%
                        background: {
                            graphic: 'https://example.com/background-image.jpg', // background-graphic
                            display: 'print'
                        }
                    };

                    // تنفيذ طباعة الصفحة مع الإعدادات
                    window.print(printOptions);
                }
            </script>
            <style>
                @media print {
                    .bcont {
                        display: none !important;
                    }

                    aside {
                        display: none !important;
                    }

                    header {
                        display: none !important;
                    }

                    .inmain {
                        width: 95% !important;
                    }

                    .container {
                        width: 100% !important;
                    }

                    button {
                        display: none;
                    }

                    /* تعيين الصفحة للطباعة في وضع landscape */
                    @page {
                        size: landscape;
                    }
                }
            </style>



        </div>
</form>

</body>

</html>

<style>
    .conta {
        margin-top: 30px;
        display: flex;
        justify-content: center;
        width: block;
    }

    button {
        margin: 20px;
        width: 15%;
        padding: 10px;
        border: none;
        border-radius: 5px;
        cursor: pointer;
        background: linear-gradient(0.25turn, #66AA3E, #92C641, #66AA3E);
        color: black;
        font-family: 'Droid Arabic Kufi', Arial, sans-serif;
        font-weight: bold;
        font-family: cairo;
    }






    header {
        position: fixed !important;
        width: 100% !important;
        top: 0px;
    }

    header,
    footer {
        text-align: center;
    }

    header,
    footer,
    aside,
    main {
        background-color: #f0f0f0;
    }

    header {
        height: 130px;
        background-color: #f0f0f0;
        display: flex !important;
        z-index: 100000;
        position: relative;
        box-shadow: 0 0 10px black;
        font-family: cairo;
    }

    .v {
        filter: unset !important;
    }

    .b1 {
        font-size: 18px;
        width: 100px;
        height: 130px;
        color: black;
        vertical-align: middle;
        clear: both;
        text-align: center;
        display: flex;
        justify-content: center;
        align-items: center;
        transition: 0.4s;
        -webkit-transition-duration: .4s;
        transition-duration: 0.4s;
        font-family: cairo;
    }

    .v {
        font-weight: bold;
        background-color: #f0f0f0;
        filter: drop-shadow(2px -10px 6px black);
    }

    .logoi {
        filter: unset !important;
    }

    .logoi {
        height: 90%;
        filter: drop-shadow(0px 0px 4px black);
    }

    img {
        width: 100%;
        height: auto;
    }

    user agent stylesheet img {
        overflow-clip-margin: content-box;
        overflow: clip;
    }

    .logo {
        justify-content: center;
        display: flex;
        left: calc(50% - 60px);
        position: absolute;
        height: 140px;
        width: 140px;
        z-index: 1;
    }

    body {
        margin-top: 20px;
        background-color: #f0f0f0;
    }
</style>